...
func TestGetContracts(t *testing.T) {
	var (
		...
		matic = &Contracts{
			...
			FeeModuleV1:        common.HexToAddress("0x56C79347e95530c01A2FC76E732f9566dA16E113"),
			FeeModuleV2:        common.HexToAddress("0xE3f18aCc55091e2c48d883fc8C8413319d4Ab7b0"),
	...
	assert.True(t, bytes.Equal(c.FeeModuleV1[:], amoy.FeeModuleV1[:]))
	assert.True(t, bytes.Equal(c.FeeModuleV2[:], amoy.FeeModuleV2[:]))
	assert.True(t, bytes.Equal(c.Collateral[:], amoy.Collateral[:]))
	assert.True(t, bytes.Equal(c.Conditional[:], amoy.Conditional[:]))
	assert.True(t, bytes.Equal(c.NegRiskFeeModuleV1[:], amoy.NegRiskFeeModuleV1[:]))
	assert.True(t, bytes.Equal(c.NegRiskFeeModuleV2[:], amoy.NegRiskFeeModuleV2[:]))
	...
	assert.True(t, bytes.Equal(c.FeeModuleV1[:], matic.FeeModuleV1[:]))
	assert.True(t, bytes.Equal(c.FeeModuleV2[:], matic.FeeModuleV2[:]))
	assert.True(t, bytes.Equal(c.Collateral[:], matic.Collateral[:]))
	assert.True(t, bytes.Equal(c.Conditional[:], matic.Conditional[:]))
	assert.True(t, bytes.Equal(c.NegRiskFeeModuleV1[:], matic.NegRiskFeeModuleV1[:]))
	assert.True(t, bytes.Equal(c.NegRiskFeeModuleV2[:], matic.NegRiskFeeModuleV2[:]))
