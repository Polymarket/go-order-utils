...
var LIMIT_ORDER_PROTOCOL_TYPE_HASH [32]byte = crypto.Keccak256Hash(
	[]byte("LimitOrder(uint256 salt,address makerAsset,address takerAsset,bytes makerAssetData,bytes takerAssetData,bytes getMakerAmount,bytes getTakerAmount,bytes predicate,address signer,uint256 sigType)"),
)
